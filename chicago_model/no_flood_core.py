...
class ScenarioConfig:
    ...
    initial_soc_fractions: Tuple[float, ...] = (0.1, 0.2, 0.3, 0.4)
...
def hybrid_explicit_logit_loading(
    ...
    eligible = np.ones(16, dtype=np.uint8)
    station_seen = np.zeros(max(1, n_stations), dtype=np.uint8)
    station_allowed = np.zeros(max(1, n_stations), dtype=np.uint8)
...
OUTPUT_ROOT = DATA_ROOT / "ChicagoRegional_flood_kept_only_outputs"

ANALYSIS_HOURS = 3.0
BATTERY_RANGE_MILES = 235.0
INITIAL_SOC_FRACTIONS = (0.1, 0.2, 0.3, 0.4)
EV_CLASS_SHARES = (0.25, 0.25, 0.25, 0.25)
THETA_EV = 0.2
THETA_NON_EV = 0.2
ALPHA_BPR = 0.15
BETA_BPR = 4.0
MAX_MSA_ITERATIONS = 1200
MIN_MSA_ITERATIONS = 10
MSA_TOLERANCE = 1.0e-4
DEMAND_SCALE = 1.0
PROGRESS_EVERY = 10
...
EV_PRIORITY_STATION_ID = "C6"
EV_REQUIRE_PRIORITY_STATION_IF_FEASIBLE = True
EV_DIRECT_CANDIDATE_TREES = 10
EV_DIRECT_PERTURBATION_STRENGTH = 0.55


HYBRID_YEN_ENABLED = True
HYBRID_YEN_INITIAL_MAX_RECORDS = 1000
HYBRID_YEN_CUMULATIVE_DEMAND_COVERAGE = 0.85
HYBRID_YEN_SEARCH_K = 8
HYBRID_YEN_OVERLAP_TRIGGER = 0.80
HYBRID_YEN_MINIMUM_STATION_CHOICES = 2
HYBRID_YEN_CORRIDOR_EXPANSIONS = (0, 1, 2)
HYBRID_YEN_ALLOW_FULL_GRAPH_FALLBACK = False
HYBRID_YEN_MAX_STATIONS_PER_RECORD = 3
HYBRID_YEN_N_JOBS = 8
HYBRID_YEN_PROGRESS_EVERY = 100
HYBRID_YEN_ENRICH_EVERY = 20
HYBRID_YEN_ENRICH_TOP_M = 50
HYBRID_YEN_CACHE_INITIAL_POOL = True
HYBRID_YEN_REBUILD_CACHE = False

RUN_MODEL = True

HYBRID_YEN_SETTINGS = HybridYenSettings(
    enabled=bool(HYBRID_YEN_ENABLED),
    initial_max_records=int(HYBRID_YEN_INITIAL_MAX_RECORDS),
    cumulative_demand_coverage=float(HYBRID_YEN_CUMULATIVE_DEMAND_COVERAGE),
    search_k=int(HYBRID_YEN_SEARCH_K),
    overlap_trigger=float(HYBRID_YEN_OVERLAP_TRIGGER),
    minimum_station_choices=int(HYBRID_YEN_MINIMUM_STATION_CHOICES),
    corridor_expansions=tuple(int(x) for x in HYBRID_YEN_CORRIDOR_EXPANSIONS),
    allow_full_graph_fallback=bool(HYBRID_YEN_ALLOW_FULL_GRAPH_FALLBACK),
    max_stations_per_record=int(HYBRID_YEN_MAX_STATIONS_PER_RECORD),
    n_jobs=int(HYBRID_YEN_N_JOBS),
    progress_every=int(HYBRID_YEN_PROGRESS_EVERY),
    enrich_every=int(HYBRID_YEN_ENRICH_EVERY),
    enrich_top_m=int(HYBRID_YEN_ENRICH_TOP_M),
    cache_initial_pool=bool(HYBRID_YEN_CACHE_INITIAL_POOL),
    rebuild_cache=bool(HYBRID_YEN_REBUILD_CACHE),
)

CONFIG = ScenarioConfig(
    scenario_id="chicago_no_flood_on_flood_kept_topology_hybrid_tree_yen_stronger_queue_penalty",
    scenario_label="No flooding on flood-kept topology — hybrid Yen with stronger queue-choice penalty",
    ...
    max_msa_iterations=int(MAX_MSA_ITERATIONS),
    min_msa_iterations=int(MIN_MSA_ITERATIONS),
    msa_tolerance=float(MSA_TOLERANCE),
    demand_scale=float(DEMAND_SCALE),
    print_all_links=bool(PRINT_ALL_LINKS),
    ...
    ev_priority_station_id=str(EV_PRIORITY_STATION_ID),
    ev_require_priority_station_if_feasible=bool(EV_REQUIRE_PRIORITY_STATION_IF_FEASIBLE),
    ev_direct_candidate_trees=int(EV_DIRECT_CANDIDATE_TREES),
    ev_direct_perturbation_strength=float(EV_DIRECT_PERTURBATION_STRENGTH),
    ev_yen_search_k=int(HYBRID_YEN_SEARCH_K),
    ev_yen_n_jobs=int(HYBRID_YEN_N_JOBS),
    ev_yen_rebuild_cache=bool(HYBRID_YEN_REBUILD_CACHE),
    ev_yen_progress_every=int(HYBRID_YEN_PROGRESS_EVERY),
)
...
def run(data_root=None, output_root=None):
    ...
    output_dir = OUTPUT_ROOT / CONFIG.scenario_id
    if NUMBA_AVAILABLE:
        try:
    ...
    print(f"Data search root: {DATA_ROOT}")
    print(f"Output directory: {output_dir}")
    print(f"Selected scenario: {CONFIG.scenario_label}")
    print(
        "Queue choice penalty: "
        f"wait multiplier={CONFIG.queue_wait_disutility_multiplier:.2f}, "
        f"utilization barrier starts at {CONFIG.queue_utilization_penalty_start:.2f}, "
        f"scale={CONFIG.queue_utilization_penalty_scale_min:.2f} min."
    )
    print(f"Hybrid Yen settings: {HYBRID_YEN_SETTINGS}")
    result = run_scenario(CONFIG, DATA_ROOT, output_dir)
    link_results = build_link_table(result)
    station_results = build_station_table(result)
    summary_results = pd.DataFrame([compute_time_summary(result)])
    ev_path_pool_results = result.ev_path_pool_summary.copy() if result.ev_path_pool_summary is not None else pd.DataFrame()
    hybrid_yen_results = getattr(result, "hybrid_yen_summary", pd.DataFrame())
    hybrid_yen_records = getattr(result, "hybrid_yen_records", pd.DataFrame())
    try:
        ...
        print("\nFast K-route diagnostics:")
        display(ev_path_pool_results)
        print("\nHybrid Yen diagnostics:")
        display(hybrid_yen_results)
        print("\nFirst 20 refined records:")
        display(hybrid_yen_records.head(20))
        print("\nFirst 20 link-flow rows:")
        display(link_results.head(20))
    except Exception:
        print(summary_results.to_string(index=False))
        print(station_results.to_string(index=False))
        print(hybrid_yen_results.to_string(index=False))
    return {
        "result": result,
        "link_results": link_results,
        "station_results": station_results,
        "summary_results": summary_results,
        "ev_path_pool_results": ev_path_pool_results,
        "hybrid_yen_results": hybrid_yen_results,
        "hybrid_yen_records": hybrid_yen_records,
