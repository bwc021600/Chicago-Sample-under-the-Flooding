...
class ScenarioConfig:
    ...
    station_utilization_screen_enabled: bool = True
    station_utilization_threshold: float = 0.20
    station_utilization_fallback_band: float = 0.02
...
def validate_queue_choice_settings(config: ScenarioConfig) -> None:
    ...
    if not (0.0 <= config.station_utilization_threshold < 1.0):
        raise ValueError("station_utilization_threshold must be in [0, 1).")
    if config.station_utilization_fallback_band < 0.0:
        raise ValueError("station_utilization_fallback_band must be nonnegative.")
    ...
    for ci in range(n_classes):
        ...
        for oi in range(n_orig):
            for dj in range(n_dest):
                ...
                cheapest_station_is_novel = (
                    cheapest_idx >= 0
                    and cand_station[cheapest_idx] >= 0
                    and used_station[cand_station[cheapest_idx]] == 0
                )
                preserve_cheapest_now = (
                    direct_possible
                    or not prefer_distinct_stations
                    or cand_station[cheapest_idx] < 0
                    or cheapest_station_is_novel
                ) if cheapest_idx >= 0 else False
                if (
                    preserve_cheapest_now
                    and selected[cheapest_idx] == 0
                    and selected_count < k_paths
                ):
                ...
                if not direct_possible and prefer_distinct_stations:
                    ...
                    for sj in range(n_stations):
                        station_has_candidate = False
                        for aa in range(n_alt):
                            if cand_station[aa] == sj:
                                station_has_candidate = True
                                break
                        if station_has_candidate:
                            feasible_station_count += 1
                    expected_distinct = min(k_paths, feasible_station_count)
                    if distinct < expected_distinct:
                        raise RuntimeError(
                            "Station-diverse K-path invariant failed in fast tree pool"
                        )
...
def fast_tree_ev_logit_loading(
    ...
    utilization_screen_enabled: bool,
    utilization_threshold: float,
    utilization_fallback_band: float,
    unstable_loading_penalty_min: float,
) -> Tuple[
    np.ndarray, np.ndarray, np.ndarray, np.ndarray,
    np.ndarray, np.ndarray, np.ndarray
    ...
    station_seen = np.zeros(max(1, n_stations), dtype=np.uint8)
    station_allowed = np.zeros(max(1, n_stations), dtype=np.uint8)
    ...
    for ci in range(n_classes):
        ...
        for oi in range(n_orig):
            for dj in range(n_dest):
                ...
                for kk in range(count):
                    ...
                    if kind == 0:
                        ...
                        if (
                            ...
                            and math.isfinite(sojourn)
                            and m1 <= r0 + 1.0e-9
                            and m2 <= full_range + 1.0e-9
                        ):
                            costs[kk] = c1 + c2 + sojourn
                            valid[kk] = 1
                            n_valid += 1
                ...
                if utilization_screen_enabled:
                    for sj in range(n_stations):
                        station_seen[sj] = 0
                        station_allowed[sj] = 0
                    distinct_station_count = 0
                    for kk in range(count):
                        if valid[kk] != 1 or int(pool_kind[ci, oi, dj, kk]) != 1:
                            continue
                        sj = int(pool_station[ci, oi, dj, kk])
                        if 0 <= sj < n_stations and station_seen[sj] == 0:
                            station_seen[sj] = 1
                            distinct_station_count += 1

                    if distinct_station_count > 1:
                        under_threshold_count = 0
                        minimum_utilization = np.inf
                        for sj in range(n_stations):
                            if station_seen[sj] == 0:
                                continue
                            util = station_utilization[sj]
                            if math.isfinite(util):
                                if util < minimum_utilization:
                                    minimum_utilization = util
                                if util <= utilization_threshold + 1.0e-12:
                        ...
                        if under_threshold_count > 0:
                            for sj in range(n_stations):
                                ...
                                if (
                                    ...
                                    and util <= utilization_threshold + 1.0e-12
                                ):
                                    station_allowed[sj] = 1
                        elif math.isfinite(minimum_utilization):
                            cutoff = minimum_utilization + utilization_fallback_band
                            for sj in range(n_stations):
                                util = station_utilization[sj]
                                if (
                                    station_seen[sj] == 1
                                    and math.isfinite(util)
                                    and util <= cutoff + 1.0e-12
                                ):
                                    station_allowed[sj] = 1
                        ...
                        for kk in range(count):
                            if valid[kk] != 1 or int(pool_kind[ci, oi, dj, kk]) != 1:
                                continue
                            sj = int(pool_station[ci, oi, dj, kk])
                            if sj < 0 or sj >= n_stations or station_allowed[sj] == 0:
                                valid[kk] = 0
                                n_valid -= 1
                            else:
                                allowed_count += 1
                        if allowed_count <= 0:
                            ...
                            best_util = np.inf
                            for sj in range(n_stations):
                                if station_seen[sj] == 1 and station_utilization[sj] < best_util:
                                    best_station = sj
                                    best_util = station_utilization[sj]
                            n_valid = 0
                            for kk in range(count):
                                if (
                                    int(pool_kind[ci, oi, dj, kk]) == 1
                                    and int(pool_station[ci, oi, dj, kk]) == best_station
                                    and math.isfinite(costs[kk])
                                ):
                                    valid[kk] = 1
                                    n_valid += 1
                ...
                cmin = np.inf
                for kk in range(count):
                    if valid[kk] == 1 and math.isfinite(costs[kk]) and costs[kk] < cmin:
                        cmin = costs[kk]
                ...
                for kk in range(count):
                    if weights[kk] <= 0.0:
                        continue
                    flow = q * weights[kk] / denom
                    kind = int(pool_kind[ci, oi, dj, kk])
                    if kind == 0:
                        vv = int(pool_direct_tree[ci, oi, dj, kk])
                        direct_q[vv, oi, dj] += flow
                    else:
                        sj = int(pool_station[ci, oi, dj, kk])
                        fv = int(pool_first_tree[ci, oi, dj, kk])
                        sv = int(pool_second_tree[ci, oi, dj, kk])
                        first_q[fv, oi, sj] += flow
                        second_q[sv, sj, dj] += flow
                        station_entries[sj] += flow
...
def perform_assignment(
    ...
    for iteration in range(1, config.max_msa_iterations + 1):
        ...
        (
            ...
            qmetrics.utilization[operational_station_indices],
            config.station_utilization_screen_enabled,
            config.station_utilization_threshold,
            config.station_utilization_fallback_band,
            config.queue_unstable_loading_penalty_min,
        )
...
def _hybrid_select_station_diverse(
    ...
    def add(path: HybridCandidatePath) -> bool:
        ...
        selected_sets.append(_hybrid_road_edge_set(path, road_edge_mask))
        return True
    ...
    uncovered = {
        ...
        if station not in {int(path.station_local_index) for path in selected}
    }
    while uncovered and len(selected) < k_target:
        if not selected:
            station, path = min(
                uncovered.items(),
                key=lambda item: (item[1].base_cost_min, item[1].distance_miles),
            )
        else:
            reference = max(
                1.0,
                min(path.base_cost_min for path in uncovered.values()),
            )
            station, path = max(
                uncovered.items(),
                key=lambda item: (
                    diversity_weight * min(
                        _hybrid_jaccard_distance(
                            _hybrid_road_edge_set(item[1], road_edge_mask), existing
                        )
                        for existing in selected_sets
                    )
                    - cost_weight * (item[1].base_cost_min / reference),
                    -item[1].base_cost_min,
                ),
            )
        add(path)
        uncovered.pop(station, None)
    ...
    while remaining and len(selected) < k_target:
        reference = max(
            1.0,
            min((path.base_cost_min for path in remaining), default=1.0),
        )
        best_index = -1
        best_score = -1.0e300
        best_cost = np.inf
        for index, path in enumerate(remaining):
            path_set = _hybrid_road_edge_set(path, road_edge_mask)
            minimum_distance = (
                min(_hybrid_jaccard_distance(path_set, existing) for existing in selected_sets)
                if selected_sets else 1.0
            )
            score = (
                diversity_weight * minimum_distance
                - cost_weight * (path.base_cost_min / reference)
            )
            if score > best_score + 1.0e-12 or (
                abs(score - best_score) <= 1.0e-12
                and path.base_cost_min < best_cost
    ...
    expected_distinct = min(int(k_target), len(nonempty))
    actual_distinct = len({int(path.station_local_index) for path in selected})
    if actual_distinct < expected_distinct:
        raise RuntimeError(
            "Station-diverse K-path invariant failed after selective Yen refinement."
        )
...
def hybrid_explicit_logit_loading(
    ...
    eligible = np.zeros(16, dtype=np.uint8)
    station_seen = np.zeros(max(1, n_stations), dtype=np.uint8)
    station_allowed = np.zeros(max(1, n_stations), dtype=np.uint8)
    ...
    for rr in range(record_demand.size):
        ...
        if utilization_screen_enabled:
            ...
            for local in range(count):
                if eligible[local] == 0:
                    continue
                station = int(path_station[start + local])
                if 0 <= station < n_stations and station_seen[station] == 0:
            ...
            if distinct_station_count > 1:
                ...
                for local in range(count):
                    if eligible[local] == 0:
                        continue
                    station = int(path_station[start + local])
                    if station >= 0 and station_allowed[station] == 0:
                        eligible[local] = 0
                    else:
                        eligible_after += 1
                if eligible_after <= 0:
        ...
        cmin = np.inf
        for local in range(count):
            if eligible[local] == 1 and costs[local] < cmin:
                cmin = costs[local]
        if not math.isfinite(cmin):
            unserved_by_class[ci] += q
            continue
        ...
        for local in range(count):
            ...
            weights[local] = 0.0 if exponent < -745.0 else math.exp(exponent)
            denom += weights[local]
        ...
        if denom <= 0.0 or not math.isfinite(denom):
            ...
            best_cost = np.inf
            for local in range(count):
                if eligible[local] == 1 and costs[local] < best_cost:
                    best = local
...
OUTPUT_ROOT = DATA_ROOT / "ChicagoRegional_flood_kept_only_C3_C8_C9_budget_45M_outputs"
...
STATION_UTILIZATION_SCREEN_ENABLED = True
STATION_UTILIZATION_THRESHOLD = 0.20
STATION_UTILIZATION_FALLBACK_BAND = 0.02
...
EV_PATH_REFRESH_EVERY_SEARCH = 0
EV_PATH_REFRESH_EVERY_POLISH = 10
...
BIHMH_STATION_MARGINAL_GATE_ENABLED = True
BIHMH_STATION_MIN_NEW_ENTRIES_PER_3HR = 5.0
BIHMH_STATION_MIN_AVG_WAIT_REDUCTION_MIN = 0.25
BIHMH_STATION_MIN_AVG_DELAY_REDUCTION_MIN = 0.50
BIHMH_STATION_MIN_TOTAL_WAIT_REDUCTION_PCT = 0.02
BIHMH_STATION_FEASIBILITY_IMPROVEMENT_EPS = 1.0e-8
BIHMH_STATION_PRUNE_OPERATOR_BOOST_AFTER_TARGET = 4.0
BIHMH_INITIAL_STATION_PRUNE_TRIALS = 6
...
class PlanEvaluation:
    ...
    charging_entries_period: float
    average_wait_min_per_charging_session: float
    average_service_min_per_charging_session: float
    average_station_delay_min_per_charging_session: float
    station_entries_by_id: Tuple[Tuple[str, float], ...]
...
class BIHMHGenerationRecord:
    ...
    best_charging_entries_period: float
    best_average_wait_min_per_charging_session: float
    best_average_station_delay_min_per_charging_session: float
...
def save_current_best_checkpoint(
    *,
    ...
    payload = {
        ...
        "charging_entries_period": float(best.charging_entries_period),
        "average_wait_min_per_charging_session": float(
            best.average_wait_min_per_charging_session
        ),
        "average_service_min_per_charging_session": float(
            best.average_service_min_per_charging_session
        ),
        "average_station_delay_min_per_charging_session": float(
            best.average_station_delay_min_per_charging_session
        ),
        "station_entries_by_id": {
            str(station): float(entries)
            for station, entries in best.station_entries_by_id
        },
...
FOCUS_MODE_TEMPLATE = SearchMode(
    ...
    path_refresh_every=int(EV_PATH_REFRESH_EVERY_SEARCH),
...
POLISH_MODE = SearchMode(
    ...
    tree_count=int(EV_DIRECT_CANDIDATE_TREES_FINAL),
    path_refresh_every=int(EV_PATH_REFRESH_EVERY_POLISH),
...
def _base_assignment_config() -> ScenarioConfig:
    return ScenarioConfig(
        scenario_id="chicago_flood_kept_C3_C8_C9_budget_45m_bihmh_marginal_station_diverse_k5",
        ...
        station_utilization_screen_enabled=bool(STATION_UTILIZATION_SCREEN_ENABLED),
        station_utilization_threshold=float(STATION_UTILIZATION_THRESHOLD),
        station_utilization_fallback_band=float(STATION_UTILIZATION_FALLBACK_BAND),
        ev_initial_k_paths=int(EV_INITIAL_K_PATHS),
        ev_path_refresh_every=int(EV_PATH_REFRESH_EVERY_SEARCH),
...
def mode_config_for_plan(plan: RepairPlan, mode: SearchMode) -> ScenarioConfig:
    ...
    return _dc_replace(
        BASE_ASSIGNMENT_CONFIG,
        scenario_id=f"flood_kept_C3_C8_C9_budget45m_bihmh_{mode.name}_{digest}",
        ...
        ev_path_refresh_every=int(mode.path_refresh_every),
...
def _finite_evaluation_metrics(result: AssignmentResult) -> Dict[str, float]:
    ...
    charging_entries_period = float(np.sum(entries[operational]))
    average_wait_min = (
        wait_vehicle_min / charging_entries_period
        if charging_entries_period > float(QUEUE_FEASIBILITY_TOLERANCE)
        else 0.0
    )
    average_service_min = (
        service_vehicle_min / charging_entries_period
        if charging_entries_period > float(QUEUE_FEASIBILITY_TOLERANCE)
        else 0.0
    )
    average_station_delay_min = average_wait_min + average_service_min
    ...
    return {
        ...
        "charging_entries_period": charging_entries_period,
        "average_wait_min_per_charging_session": average_wait_min,
        "average_service_min_per_charging_session": average_service_min,
        "average_station_delay_min_per_charging_session": average_station_delay_min,
...
def evaluate_plan(
    *,
    ...
    evaluation = PlanEvaluation(
        ...
        charging_entries_period=float(finite["charging_entries_period"]),
        average_wait_min_per_charging_session=float(
            finite["average_wait_min_per_charging_session"]
        ),
        average_service_min_per_charging_session=float(
            finite["average_service_min_per_charging_session"]
        ),
        average_station_delay_min_per_charging_session=float(
            finite["average_station_delay_min_per_charging_session"]
        ),
        station_entries_by_id=tuple(
            (str(station_id), float(result.station_metrics.entries_period[index]))
            for index, station_id in enumerate(result.network.station_ids)
        ),
        objective_key=key,
...
def generate_station_seed_plans(
    ...
    for rank, raw_subset in enumerate(EXPLICIT_STATION_WARMSTART_PACKAGES, start=1):
        ...
        if station_cost > BUDGET_USD + 1.0e-9:
            raise RuntimeError(f"Requested station combination exceeds the budget: {key}")
...
def _station_entry_map(evaluation: PlanEvaluation) -> Dict[str, float]:
    return {
        str(station).upper(): float(entries)
        for station, entries in evaluation.station_entries_by_id
    }


def _marginal_station_addition_is_acceptable(
    parent: PlanEvaluation,
    child: PlanEvaluation,
) -> Tuple[bool, str]:

    if not bool(BIHMH_STATION_MARGINAL_GATE_ENABLED):
        return True, "gate_disabled"

    parent_stations = set(parent.plan.repaired_stations)
    child_stations = set(child.plan.repaired_stations)
    added = sorted(child_stations - parent_stations)
    if not added:
        return True, "no_station_addition"

    if not parent.hard_feasible:
        improvement = float(parent.feasibility_violation - child.feasibility_violation)
        ev_gain = float(child.served_ev - parent.served_ev)
        unstable_gain = int(parent.unstable_station_count - child.unstable_station_count)
        if (
            child.hard_feasible
            or improvement > float(BIHMH_STATION_FEASIBILITY_IMPROVEMENT_EPS)
            or ev_gain > float(EV_TARGET_TOLERANCE_TRIPS)
            or unstable_gain > 0
        ):
            return True, "pre_feasibility_improvement"
        return False, "station_addition_does_not_improve_feasibility"

    if not child.hard_feasible:
        return False, "feasible_parent_to_infeasible_child"

    entry_map = _station_entry_map(child)
    minimum_added_entries = min(
        (entry_map.get(str(station).upper(), 0.0) for station in added),
        default=0.0,
    )
    if minimum_added_entries + 1.0e-12 < float(
        BIHMH_STATION_MIN_NEW_ENTRIES_PER_3HR
    ):
        return False, (
            "new_station_underused:"
            f"min_entries={minimum_added_entries:.6f}"
        )

    wait_reduction = float(
        parent.average_wait_min_per_charging_session
        - child.average_wait_min_per_charging_session
    )
    delay_reduction = float(
        parent.average_station_delay_min_per_charging_session
        - child.average_station_delay_min_per_charging_session
    )
    total_wait_reduction = float(
        parent.physical_wait_vehicle_min - child.physical_wait_vehicle_min
    )
    total_wait_reduction_pct = (
        total_wait_reduction / max(parent.physical_wait_vehicle_min, 1.0e-9)
        if parent.physical_wait_vehicle_min > 1.0e-9 else 0.0
    )

    material = bool(
        wait_reduction >= float(BIHMH_STATION_MIN_AVG_WAIT_REDUCTION_MIN)
        or delay_reduction >= float(BIHMH_STATION_MIN_AVG_DELAY_REDUCTION_MIN)
        or (
            total_wait_reduction_pct
            >= float(BIHMH_STATION_MIN_TOTAL_WAIT_REDUCTION_PCT)
            and wait_reduction > 0.0
        )
    )
    if material:
        return True, (
            "material_charging_improvement:"
            f"wait_drop={wait_reduction:.6f};"
            f"delay_drop={delay_reduction:.6f};"
            f"total_wait_drop_pct={total_wait_reduction_pct:.6f};"
            f"min_new_entries={minimum_added_entries:.6f}"
        )
    return False, (
        "no_material_charging_improvement:"
        f"wait_drop={wait_reduction:.6f};"
        f"delay_drop={delay_reduction:.6f};"
        f"total_wait_drop_pct={total_wait_reduction_pct:.6f};"
        f"min_new_entries={minimum_added_entries:.6f}"
...
class BIHMHSearch:
    ...
    def __init__(
        ...
        *,
        ...
        self.station_ids = sorted(self.station_costs, key=lambda x: int(x[1:]))
        self.all_components = [
            ("road", rid) for rid in self.road_ids
        ] + [("station", sid) for sid in self.station_ids]
        self.ops = [
            "add_fix", "station_capacity_fix", "station_prune_fix",
            "remove_fix", "swap_fix", "crossover_fix", "mip_intensify",
        ]
    ...
    def pick_operator(self, parent: PlanEvaluation, stable_generations: int) -> str:
        ...
        if not parent.hard_feasible:
            ...
            adjusted["station_prune_fix"] *= 0.25
            ...
            adjusted["station_prune_fix"] *= float(
                BIHMH_STATION_PRUNE_OPERATOR_BOOST_AFTER_TARGET
            )
    ...
    def station_prune_fix(
        ...
        *,
        ...
        if not stations:
            return parent.plan
        entry_map = _station_entry_map(parent)
        ranked = sorted(
            stations,
            key=lambda sid: (
                entry_map.get(str(sid).upper(), 0.0),
                self._component_value("station", sid)
                / max(self.station_costs[sid], 1.0),
            ),
        )
        rcl = ranked[: min(3, len(ranked))]
        chosen = rcl[0] if (not randomized or self.rng.random() < 0.80) else self.rng.choice(rcl)
        stations.remove(chosen)
        candidate = RepairPlan(
            frozenset(stations),
            parent.plan.repaired_roads,
        )
        return self.repair_budget(
            candidate,
            fill_budget=True,
            randomized_fill=bool(randomized),
        )
    ...
    def swap_fix(self, parent: PlanEvaluation) -> RepairPlan:
        ...
        fake = PlanEvaluation(
            ...
            charging_entries_period=parent.charging_entries_period,
            average_wait_min_per_charging_session=(
                parent.average_wait_min_per_charging_session
            ),
            average_service_min_per_charging_session=(
                parent.average_service_min_per_charging_session
            ),
            average_station_delay_min_per_charging_session=(
                parent.average_station_delay_min_per_charging_session
            ),
            station_entries_by_id=parent.station_entries_by_id,
    ...
    def generate_child(
        ...
        elif operator == "station_prune_fix":
            child = self.station_prune_fix(parent)
    ...
    def _select_initial_population(
        ...
        for index, (plan, source, surrogate) in enumerate(warm_plans, start=1):
            ...
            rows.append({
                ...
                "screen_charging_entries_period": evaluation.charging_entries_period,
                "screen_average_wait_min_per_charging_session": (
                    evaluation.average_wait_min_per_charging_session
                ),
                "screen_average_station_delay_min_per_charging_session": (
                    evaluation.average_station_delay_min_per_charging_session
                ),
        ...
        prune_trials = 0
        pruned_evaluations: List[PlanEvaluation] = []
        inadmissible_initial_keys: set[Tuple[Tuple[str, ...], Tuple[str, ...]]] = set()
        initial_gate_notes: Dict[Tuple[Tuple[str, ...], Tuple[str, ...]], str] = {}
        for parent in list(population):
            if (
                prune_trials >= int(BIHMH_INITIAL_STATION_PRUNE_TRIALS)
                or not parent.hard_feasible
                or len(parent.plan.repaired_stations) <= 3
            ):
                continue
            pruned_plan = self.station_prune_fix(parent, randomized=False)
            if pruned_plan.key() == parent.plan.key():
                continue
            prune_trials += 1
            screen_pruned = evaluate_plan(
                master=self.master, od=self.od, plan=pruned_plan, mode=SCREEN_MODE,
                road_costs=self.road_costs, station_costs=self.station_costs,
                cache_root=self.cache_root,
                source=parent.source + " / initial least-used-station prune screen",
            )
            if not screen_pruned.hard_feasible:
                initial_gate_notes[parent.plan.key()] = (
                    "extra station retained because least-used-station removal "
                    "failed the EV/queue feasibility screen"
                )
                continue
            final_pruned = evaluate_plan(
                master=self.master, od=self.od, plan=pruned_plan, mode=FINAL_MODE,
                road_costs=self.road_costs, station_costs=self.station_costs,
                cache_root=self.cache_root,
                source=parent.source + " / initial least-used-station prune",
            )
            pruned_evaluations.append(final_pruned)
            station_allowed, station_reason = (
                _marginal_station_addition_is_acceptable(final_pruned, parent)
            )
            initial_gate_notes[parent.plan.key()] = station_reason
            if not station_allowed:
                inadmissible_initial_keys.add(parent.plan.key())
                print(
                    "Initial marginal-station gate rejected "
                    f"stations={sorted(parent.plan.repaired_stations)}: "
                    f"{station_reason}"
                )

        if inadmissible_initial_keys:
            population = [
                evaluation for evaluation in population
                if evaluation.plan.key() not in inadmissible_initial_keys
            ]
        if pruned_evaluations:
            candidate_pool = list(population) + pruned_evaluations
            provisional_best = max(
                candidate_pool,
                key=lambda item: item.objective_key,
            )
            population = self.survivor_selection(
                population, pruned_evaluations, provisional_best
            )

        if initial_gate_notes:
            for row in rows:
                station_set = tuple(
                    sorted(x for x in str(row.get("station_set", "")).split(";") if x)
                )
                matching_notes = [
                    note for key, note in initial_gate_notes.items()
                    if tuple(key[0]) == station_set
                ]
                if matching_notes:
                    row["initial_station_marginal_gate"] = matching_notes[0]
            self.current_warmstart_audit = pd.DataFrame(rows)
        ...
        for generation in range(1, int(BIHMH_GENERATIONS) + 1):
            ...
            for info in focus_scored[: int(BIHMH_MAX_SCREEN_EVALS_PER_GENERATION)]:
                ...
                if best.hard_feasible and not screen_eval.hard_feasible:
                    ...
                    self.update_operator_weight(info["operator"], False)
                    continue
                parent_screen = evaluate_plan(
                    master=self.master, od=self.od, plan=info["parent"].plan,
                    mode=SCREEN_MODE,
                    road_costs=self.road_costs, station_costs=self.station_costs,
                    cache_root=self.cache_root,
                    source=(
                        f"BIHMH gen {generation} parent screen for marginal station gate"
                    ),
                )
                station_allowed, station_reason = (
                    _marginal_station_addition_is_acceptable(
                        parent_screen, screen_eval
                    )
                )
                info["station_marginal_reason"] = station_reason
                if not station_allowed:
                    screened_out += 1
                    self.update_operator_weight(info["operator"], False)
                    continue
            ...
            for info in screen_scored[:full_quota]:
                ...
                parent = info["parent"]
                station_allowed, station_reason = (
                    _marginal_station_addition_is_acceptable(parent, child)
                )
                info["station_marginal_reason_final"] = station_reason
                if not station_allowed:
                    self.update_operator_weight(info["operator"], False)
                    continue
            ...
            self.history.append(BIHMHGenerationRecord(
                ...
                best_charging_entries_period=best.charging_entries_period,
                best_average_wait_min_per_charging_session=(
                    best.average_wait_min_per_charging_session
                ),
                best_average_station_delay_min_per_charging_session=(
                    best.average_station_delay_min_per_charging_session
                ),
            ...
            if generation == 1 or generation % 5 == 0:
                print(
                    ...
                    f"avg_wait={best.average_wait_min_per_charging_session:.3f}min; "
...
def evaluation_table(evaluations: Sequence[PlanEvaluation]) -> pd.DataFrame:
    ...
    for rank, evaluation in enumerate(
        ...
        rows.append({
            ...
            "charging_entries_period": evaluation.charging_entries_period,
            "average_wait_min_per_charging_session": (
                evaluation.average_wait_min_per_charging_session
            ),
            "average_service_min_per_charging_session": (
                evaluation.average_service_min_per_charging_session
            ),
            "average_station_delay_min_per_charging_session": (
                evaluation.average_station_delay_min_per_charging_session
            ),
...
def save_best_plan_outputs(
    ...
    result.config = _dc_replace(
        result.config,
        scenario_id="chicago_flood_kept_C3_C8_C9_budget_45m_bihmh_marginal_station_diverse_k5_best_plan",
        scenario_label="Flood-kept network with C3/C8/C9 baseline, marginal station-repair screening, strict station-diverse K=5 paths, full-budget road top-up, and BIHMH repairs under $45M",
    )
    ...
    payload = {
        ...
        "charging_entries_period": best.charging_entries_period,
        "average_wait_min_per_charging_session": (
            best.average_wait_min_per_charging_session
        ),
        "average_service_min_per_charging_session": (
        ...
        "warmstart_rule": (
            ...
            "Initial solutions use exactly the eight user-specified three-/four-station packages; "
            "their road portions use the existing corridor-aware fill strategies. After initialization, "
        ...
        "marginal_station_gate_enabled": bool(BIHMH_STATION_MARGINAL_GATE_ENABLED),
        "marginal_station_min_new_entries_per_3hr": float(
            BIHMH_STATION_MIN_NEW_ENTRIES_PER_3HR
        ),
        "marginal_station_min_average_wait_reduction_min": float(
            BIHMH_STATION_MIN_AVG_WAIT_REDUCTION_MIN
        ),
        "marginal_station_min_average_delay_reduction_min": float(
            BIHMH_STATION_MIN_AVG_DELAY_REDUCTION_MIN
        ),
        "marginal_station_min_total_wait_reduction_pct": float(
            BIHMH_STATION_MIN_TOTAL_WAIT_REDUCTION_PCT
        ),
        "station_utilization_screen_enabled": bool(
            STATION_UTILIZATION_SCREEN_ENABLED
        ),
        "station_utilization_screen_threshold": float(
            STATION_UTILIZATION_THRESHOLD
        ),
        "station_utilization_fallback_band": float(
            STATION_UTILIZATION_FALLBACK_BAND
        ),
        "ev_k_paths": int(EV_INITIAL_K_PATHS),
        "ev_path_refresh_every_search": int(EV_PATH_REFRESH_EVERY_SEARCH),
        "ev_path_refresh_every_polish": int(EV_PATH_REFRESH_EVERY_POLISH),
        "ev_prefer_distinct_stations": bool(EV_PREFER_DISTINCT_STATIONS),
...
def _run_budget_bihmh_search_impl() -> Dict[str, _Any]:
    output_dir = OUTPUT_ROOT / "chicago_flood_kept_C3_C8_C9_budget_45m_bihmh_marginal_station_diverse_k5_checkpointed"
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    print(
        ...
        "three-/four-station warm starts are filled with the existing corridor-aware "
        "road strategies. Every BIHMH child is then topped up with roads until no "
    ...
    if POLISH_WINNER:
        ...
        if best is None:
            raise RuntimeError(
                "No evaluated $45M plan satisfied the requirements that at least "
    ...
    print(
        "Weighted average station wait: "
        f"{best.average_wait_min_per_charging_session:.6f} min/session"
    )
    print(
        "Weighted average station delay (service + wait): "
        f"{best.average_station_delay_min_per_charging_session:.6f} min/session"
    )
