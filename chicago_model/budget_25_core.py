...
OUTPUT_ROOT = DATA_ROOT / "ChicagoRegional_flood_kept_only_C3_C8_C9_budget_25M_outputs"
...
def _base_assignment_config() -> ScenarioConfig:
    return ScenarioConfig(
        scenario_id="chicago_flood_kept_C3_C8_C9_budget_25m_bihmh_marginal_station_diverse_k5",
...
def mode_config_for_plan(plan: RepairPlan, mode: SearchMode) -> ScenarioConfig:
    ...
    return _dc_replace(
        BASE_ASSIGNMENT_CONFIG,
        scenario_id=f"flood_kept_C3_C8_C9_budget25m_bihmh_{mode.name}_{digest}",
...
def save_best_plan_outputs(
    ...
    result.config = _dc_replace(
        result.config,
        scenario_id="chicago_flood_kept_C3_C8_C9_budget_25m_bihmh_marginal_station_diverse_k5_best_plan",
        scenario_label="Flood-kept network with C3/C8/C9 baseline, marginal station-repair screening, strict station-diverse K=5 paths, full-budget road top-up, and BIHMH repairs under $25M",
    )
...
def _run_budget_bihmh_search_impl() -> Dict[str, _Any]:
    output_dir = OUTPUT_ROOT / "chicago_flood_kept_C3_C8_C9_budget_25m_bihmh_marginal_station_diverse_k5_checkpointed"
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    if POLISH_WINNER:
        ...
        if best is None:
            raise RuntimeError(
                "No evaluated $25M plan satisfied the requirements that at least "
