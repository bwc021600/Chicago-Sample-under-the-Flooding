...
OUTPUT_ROOT = DATA_ROOT / "ChicagoRegional_flood_kept_only_C3_C8_C9_budget_15M_outputs"
...
def _base_assignment_config() -> ScenarioConfig:
    return ScenarioConfig(
        scenario_id="chicago_flood_kept_C3_C8_C9_budget_15m_bihmh_explicit_station_warmstarts_full_budget",
...
def mode_config_for_plan(plan: RepairPlan, mode: SearchMode) -> ScenarioConfig:
    ...
    return _dc_replace(
        BASE_ASSIGNMENT_CONFIG,
        scenario_id=f"flood_kept_C3_C8_C9_budget15m_bihmh_{mode.name}_{digest}",
...
def save_best_plan_outputs(
    ...
    result.config = _dc_replace(
        result.config,
        scenario_id="chicago_flood_kept_C3_C8_C9_budget_15m_bihmh_explicit_station_warmstarts_full_budget_best_plan",
        scenario_label="Flood-kept network with C3/C8/C9 baseline, explicit station warm starts, full-budget road top-up, and BIHMH repairs under $15M",
    )
...
def _run_budget_bihmh_search_impl() -> Dict[str, _Any]:
    output_dir = OUTPUT_ROOT / "chicago_flood_kept_C3_C8_C9_budget_15m_bihmh_explicit_station_warmstarts_full_budget_checkpointed"
    output_dir.mkdir(parents=True, exist_ok=True)
    ...
    if POLISH_WINNER:
        ...
        if best is None:
            raise RuntimeError(
                "No evaluated $15M plan satisfied the requirements that at least "
